...
struct btrfs_receive
{
	...
	// stream objects to reuse for optional decompression of encoded_write
...
static int decompress_zlib(struct btrfs_receive *rctx, const void *encoded_data,
		       u64 encoded_len, char *unencoded_data, u64 unencoded_len)
{
	int status = 0;
	int init = 0;
	int ret;
...
static int decompress_zstd(struct btrfs_receive *rctx, const void *encoded_buf,
		       u64 encoded_len, char *unencoded_buf, u64 unencoded_len)
{
...
static int decompress_and_write(const void *encoded_data, u64 encoded_len,
			    ...
			    u64 unencoded_offset, u32 compression, void *user)
{
	int ret = 0;
	size_t pos;
	size_t w;
...
static int cmd_receive(const struct cmd_struct *cmd, int argc, char **argv)
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "CevqDf:m:E:", long_opts, NULL);
		if (c < 0)
			break;
