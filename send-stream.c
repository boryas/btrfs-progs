...
static void free_read_buf(struct btrfs_send_stream *sctx)
{
	int tlv_type;

	sctx->read_buf_sz = 0;
	free(sctx->read_buf);
	/* NULL all the pointers in sctx which refer to read_buf */
	sctx->read_buf = NULL;
	for (tlv_type = 0; tlv_type < BTRFS_SEND_A_MAX + 1; ++tlv_type)
		sctx->cmd_attrs[tlv_type].data = NULL;
}
...
static int read_cmd(struct btrfs_send_stream *sctx)
{
	...
	while (pos < cmd_len) {
		...
		size_t remaining = cmd_len - pos;

		if (remaining < sizeof(__le16 *)) {
			error("reading tlv_type would overrun send stream");
			ret = -EINVAL;
			goto out;
		}
